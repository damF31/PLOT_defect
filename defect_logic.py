...
class DefectLogic:
    ...
    def update_site_atom_inputs(self):
        ...
        self._update_entries(
            ...
            [f"Al" if i == 0 else f"Al_{i+1}" for i in range(self.app.num_network_atoms.get())],
            self.app.update_atom_site_listboxes
        )
        # Sites interstitiels
        self._update_entries(
        ...
        self._update_entries(
            ...
            [f"H" if i == 0 else f"H_{i+1}" for i in range(self.app.num_added_atoms.get())],
            self.app.update_atom_site_listboxes
        )

    def _update_entries(self, frame, entries, n, label, maxn, defaults, refresh_callback=None):
        # Sauvegarde les valeurs existantes
        ...
        for i in range(n):
            ...
            if refresh_callback:
                entry.bind("<FocusOut>", lambda e: refresh_callback())
                entry.bind("<KeyRelease>", lambda e: refresh_callback())
